...
namespace ex10_MovieFinder2024
{
    ...
    {
        private bool isFavorite = false; //즐겨찾기인진/API로 검색한건지 false => openAPI, true =>즐겨찾기 보기
        public MainWindow()
        {
            InitializeComponent();
        }
        ...
        {
            string tmdb_apiKey = "de8e086be31f0a03e00b727ac0a0496d"; //TMDB사이트에서 제공받은 API키
            ...
            var jsonResult = JObject.Parse(result); //tpye.Parse(string)
            ...
            var jsonArray = results as JArray; //result가 json 배열이기 때문에 JArray는 List와 동일해서 foreach 사용가능

            var movieItems = new List<MovieItem>();
            foreach (var item in jsonArray)
            {
                var moiveItem = new MovieItem()
                {
        ...
        private void TxtMovieNAme_KeyDown(object sender, KeyEventArgs e)
        {
            if(e.Key == Key.Enter)
            {
                BtnSearch_Click(sender, e); //검색 버튼클릭 이벤트 핸들러 실행
            }
        }
        ...
        private async void BtnWatchTrailer_Click(object sender, RoutedEventArgs e)
        {
            if(GrdResult.SelectedItems.Count == 0)
            {
                await this.ShowMessageAsync("에고편보기", "영화를 선택하세요");
                return;
            }
            if(GrdResult.SelectedItems.Count > 1)
            {
                await this.ShowMessageAsync("에고편보기", "영화를 하나만 선택하세요");
                return;
            }
